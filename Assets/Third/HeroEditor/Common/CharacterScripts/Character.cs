...
namespace Assets.HeroEditor.Common.CharacterScripts
{
    ...
    public class Character : CharacterBase
    {
		...
		private void MapSprites(List<SpriteRenderer> spriteRenderers, List<Sprite> sprites)
        {
            ...
            //    part.sprite = sprites.SingleOrDefault(i => i != null && i.name == part.name);
            ...
            for (int i = 0; i < spriteRenderers.Count; i++) {
                for (int j = 0; j < sprites.Count; j++) {
                    string name1 = new string(spriteRenderers[i].name.Split('[')[0].ToCharArray());
                    string name2 = new string(sprites[j].texture.ToString().Split('(')[0].ToCharArray());
                    name1 = name1.Trim();
                    name2 = name2.Trim();
                    if (name1.Equals(name2)
                        || name1.Equals(sprites[j].name.Trim())) {
                        spriteRenderers[i].sprite = sprites[j];
                    }
                }
            }
        }
