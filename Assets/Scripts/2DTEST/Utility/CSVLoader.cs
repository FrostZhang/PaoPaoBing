...
public class CSVLoader
{
    ...
    {
        inStream = new StreamReader(fileName, encoding);
        table = new List<List<string>>();
        List<string> temp = getLineContentVector();
        while (null != temp)
        {
            ...
            table.Add(tempList);
            temp = getLineContentVector();
        }
    }

    public void ReadUTF8File(string fileName)
    {
        ReadFile(fileName, Encoding.UTF8);
    }

    public void ReadGBKFile(string fileName)
    {
        ReadFile(fileName, Encoding.GetEncoding("GBK"));
    }
